...
public class SellingBinWoodBlockEntity extends AbstractSellingBinBlockEntity implements Container, Nameable, MenuProvider {

    public SellingBinWoodBlockEntity(BlockPos pos, BlockState blockState) {
        super(SELLING_BIN_WOOD_BLOCK_ENTITY.get(), pos, blockState);
    }

    @Override
    public Component getContainerName() {
        return Component.translatable("block.monetaryexchange.selling_bin_wood");
    }

    @Override
    public AbstractContainerMenu createContainerMenu(int containerId, Inventory inventory, Player player) {
        SellingBinEnderContainer c = player.getData(MonetaryExchange.SELLING_BIN_WOOD_HANDLER).Container();
        return new ChestMenu(MenuType.GENERIC_9x2, containerId, inventory, c, 2);
    }
